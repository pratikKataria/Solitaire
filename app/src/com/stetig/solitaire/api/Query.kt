...
//        SELECT COUNT(ID) FROM TASK WHERE RecordTypeId = '012C3000000Aqq9IAC' AND WhatId != null AND Call_Proposed_Date_Of_Visit__c != null
        ...
        val SITE_VISIT_LIST = "SELECT Id,Name,Customer_Name1__c,Type_of_Visit__c, Customer_Name__c, Type_of_Enquiry__c,Opportunity_Project__c,Opportunity_Sourcing_Manager__c,createdDate,Site_Visit_Stage__c from site_visit__c $WHERE $OWNER_FILTER $AND $SITE_VISIT_FILTER"
...
//        Select Id, ActivityDate, WhatId, What.Name , CreatedDate FROM Task WHERE whatid in (select id from opportunity) AND RecordTypeId = '012C3000000Aqq9IAC'
        ...
        val PROPOSED_SITE_VISIT_LIST = "SELECT ID, what.Name, what.Id,CreatedDate, Call_Proposed_Date_Of_Visit__c FROM TASK $WHERE RecordTypeId = '012C3000000Aqq9IAC' $AND Call_Proposed_Date_Of_Visit__c != null AND $OWNER_FILTER"
